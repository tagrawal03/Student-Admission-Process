...
int main (void)
{
  ...
  printf("Student 4 has ip address: %s", inet_ntoa(myaddr.sin_addr));
  ...
  //printf("Student4: UDP - waiting to recieve from ...\n");      
  ...
  cout <<"Student 4 has recieved the reply from Admission office"<<endl;
  ...
  /*Write result recieved to file  if valid application*/
  if (udpnumbytes > 2)  //check if application is valid
  {
  stufile.open("Student4_result.txt");
  stufile << data <<endl;
  ...
  printf("Student4 has ip address: %s", inet_ntoa(myaddr.sin_addr));
  printf(" and UDP port number: %d for phase 2\n", (int) ntohs(myaddrUdp.sin_port));

  printf("Student 4 has recieved application result '%s' \n", data);
  cout << "End of phase 2 for Student 4"<<endl;
  cout <<'\n'<<endl;
