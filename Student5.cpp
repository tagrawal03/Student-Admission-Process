...
int main (void)
{
  ...
  printf("Student 5 has ip address: %s", inet_ntoa(myaddr.sin_addr));
  printf(" and TCP port number: %d \n", (int) ntohs(myaddr.sin_port));
  
  /*Sending file*/
  ifstream myfile("student5.txt");
  string line;
  if (myfile.is_open())
  {
  ...
  //printf("Student5: UDP - waiting to recieve from ...\n");      
  ...
  cout <<"Student 5 has recieved the reply from Admission office"<<endl;
  ...
  /*Wrire result recievde to file*/
  stufile.open("Student5_result.txt");
  stufile << data <<endl;
  ...
  printf("Student5 has ip address: %s", inet_ntoa(myaddr.sin_addr));
  printf(" and UDP port number: %d for phase 2\n", (int) ntohs(myaddrUdp.sin_port));


  printf("Student 5 has recieved application result '%s' \n", data);
  cout << "End of phase 2 for Student 5 "<<endl;
  cout <<'\n'<<endl;
