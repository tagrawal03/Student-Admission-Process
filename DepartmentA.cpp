...
int main (void)
{
  ...
  printf("Department A has ip address: %s", inet_ntoa(myaddr.sin_addr));
  printf(" and TCP port number: %d for phase 1 \n", (int) ntohs(myaddr.sin_port));
  
  /*Sending file*/
  ifstream myfile("departmentA.txt");
  string line;
  if (myfile.is_open())
  {
      while(getline(myfile, line))
      {
        ...
        {
           cout<<"Department A has sent " <<value<< " to the admission office"<< endl;
        }

        delete []cstr;
      }
      cout << "Updating the admission office is done for Department A" <<endl;
  ...
  if ((rv = getaddrinfo(NULL, DEPA_UDPPORT, &hintsudp, &udpservinfo)) != 0)
  {
    fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
  }
  ...
  printf("DepartmentA: Waiting for Admission result ...\n");      

  addr_len = sizeof(their_addr);
  depfile.open("DepartmentA_result.txt");
   ...
   while(1)
   {
      ...
      {
         data[udpnumbytes] = '\0';
         cout <<data<< " has been admitted to department A" <<endl;
  ...
  cout << "End of phase 2 for Department A"<<endl;
  depfile.close();
  close(sockfd3);
